...
//                      Records should not be there - fixed and running. Thanks to Jens Göhring
...
// while on 9600 Baud every 1ms a Char is received, is this time on 256KBaud only 4µs
// Strange enough, windows support only 1ms as shortest Intervall!
// Below some standard TimeOuts for the given Baudrates in µs
...
// See sample Programs for more Information
  ...
  private
    ...
    fModemState : DWord;
    // TimeOut definitions
    fRTOCharDelayTime : DWord;     // in µs max: 4.294.967.295µs aprox 1h 20min
    fRTOExtraDelayTime : Word;    // in ms
    fWTOCharDelayTime : DWord;     // in µs
